...
for i in range(51):
        cost = i+1
	RGB_video_path = '/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/5-cross/2_resize/'
	#RGB_video_path = 'crop_images/'
	SAMPLE_LABEL_PATH='/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/5-cross/sample_label_five_2.txt'
	#IMAGE_LABEL_PATH='test_four.txt'
	RESULT_LABELS_PATH='../cost-test_result/5-6model_five-2-cost'+str(cost)+'.txt'
	#RESULT_PROBS_PATH='liulin/p_four1.txt'
	SAMPLE_NUMBER=5

	lstm_model = 'deploy_resnet50_lstm.prototxt'

	RGB_lstm = '../5-6-2caffe_resnet50_train_'+str(cost)+'_iter_2000.caffemodel'
	#if len(sys.argv) > 1:
	#  video = sys.argv[1]
	#else:
	#  video = '0001'
