...
cross = ['1','3','4','5']
for i in range(4):
	RGB_video_path = '/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/5-cross/'+cross[i]+'_resize/'
	#RGB_video_path = 'crop_images/'
	SAMPLE_LABEL_PATH='/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/5-cross/sample_label_five_'+cross[i]+'.txt'
	#IMAGE_LABEL_PATH='test_four.txt'
	RESULT_LABELS_PATH='../cost-test_result/5-6model_five-'+cross[i]+'-cost8.txt'
	#RESULT_PROBS_PATH='liulin/p_four1.txt'
	SAMPLE_NUMBER=5

	lstm_model = 'deploy_resnet50_lstm.prototxt'

	RGB_lstm = '../5-6-'+cross[i]+'caffe_resnet50_train_8_iter_2000.caffemodel'
	#if len(sys.argv) > 1:
	#  video = sys.argv[1]
	#else:
	#  video = '0001'
