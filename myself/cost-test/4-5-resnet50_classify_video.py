...
cross = ['1','2','3','4','5']
for i in range(5):
	RGB_video_path = '/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/4-predict-5/'+cross[i]+'_resize/'
	#RGB_video_path = 'crop_images/'
	SAMPLE_LABEL_PATH='/home/shiyan/lisa-caffe-public-lstm_video_deploy/examples/LRCN_activity_recognition/4-predict-5/sample_label_four_'+cross[i]+'.txt'
	#IMAGE_LABEL_PATH='test_four.txt'
	RESULT_LABELS_PATH='../cost-test_result/5-6model_four-'+cross[i]+'-cost1.txt'
	#RESULT_PROBS_PATH='liulin/p_four1.txt'
	SAMPLE_NUMBER=4

	lstm_model = 'deploy_resnet50_lstm.prototxt'

	RGB_lstm = '../5-6-'+cross[i]+'caffe_resnet50_train_1_iter_2000.caffemodel'
	#if len(sys.argv) > 1:
	#  video = sys.argv[1]
	#else:
	#  video = '0001'
	...
	def initialize_transformer(image_mean, is_flow):
	  shape = (10*SAMPLE_NUMBER, 3, 112, 112)
	...
	ucf_mean_RGB=np.load('/home/shiyan/cost-lstm-caffe-master-paper/myself/resnet50_lstm/mnist_train_lmdb_mean.npy')
	...
	def LRCN_classify_video(frames, net, transformer, is_flow):
	  ...
	  for im in frames:
	    ...
	    if (input_im.shape[0] < 120):
	      input_im = caffe.io.resize_image(input_im, (120,160))
	  ...
	  print len(clip_input)
	  clip_input = caffe.io.oversample(clip_input,[112,112])
